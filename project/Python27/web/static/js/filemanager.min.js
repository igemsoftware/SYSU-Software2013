$(document).ready(function(){
	  document.getElementById('hr1').size=document.getElementById('leftcol').clientHeight*3;
          ...
          ws.onmessage = function (msg) {
			...
			}else if(message.request=='get_extended_sbol')
			{
				var blob = new Blob([message.result], {type: "text/plain;charset=utf-8"});
				saveAs(blob, "sbol.txt");
			}else if(message.request=='deleteUserPart')
			{
		...
		$("button.export").live("click", function() {		  
		  console.log(this.getAttribute("id"));
		  ws.send(JSON.stringify({'request': 'get_extended_sbol','part_id':this.getAttribute("id")}));
        });
		$("button.deletepart").live("click", function() {		  
		  ws.send(JSON.stringify({'request': 'deleteUserPart', 'part_id':this.getAttribute("id"),'uploaduser':this.getAttribute('name')}));
        });
